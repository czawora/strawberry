ConsensusClusterPlus <- function( d=NULL,
  ...
  ######added by chris
  ######this is the differnce in indexing ,starts at 2, 
  ######and the numbers of cluster in the network, starts at minNumK
  offset <- minNumK - 2
  ...
  for (tk in 2:maxK){
    if(verbose){
      message(paste("consensus ",tk + offset))
    }
    fm = ml[[tk]]
  ...
  if(writeTable){
    for(i in 2:length(res)){
      write.csv(file=paste(title,"/",title,".k=",i + offset,".consensusMatrix.csv",sep=""), res[[i]]$consensusMatrix)
      write.table(file=paste(title,"/",title,".k=",i + offset,".consensusClass.csv",sep=""), res[[i]]$consensusClass,col.names = F,sep=",")
    }
  }
  return(res)
}
...
calcICL = function(res, maxK = 3, minNumK = 2, title="untitled_consensus_cluster",plot=NULL,writeTable=FALSE){
  ...
  for (k in 2:length(res)){ #each k
	...
	cc=rbind(cc,c(k + offset,ci,mk)) #cluster-consensus
      ...
      for (ei in rev(res[[2]]$consensusTree$order) ){
		...
		cci = rbind(cci,c(k + offset,ci,ei,mei)) #cluster, cluster index, item index, item-consensus
      }
      eiCols = c(eiCols, rep(ci,length(o$consensusClass)) )
    }
	  
	  cck = cci[which(cci[,1]==k + offset),] #only plot the new k data.
	  ...
	  sumRes[[k]] = rankedBarPlot(q,thisColors,cc=res[[k]]$consensusClass[res[[2]]$consensusTree$order],paste("k=",k + offset,sep="") )
  }
...
ccRun <- function( d=d,
  ...
  for (i in 1:repCount){
    print(paste(used_mem()))
    if(verbose){
      print(paste("random subsample",i));
    }
    ...
    for (k in 2:maxK){
      ...
      if(clusterAlg=="hc"){
        ##prune to k for hc
        this_assignment = cutree(this_cluster,k + offset)
...
CDF=function(ml, offset = 0,breaks=100){
  ...
  legend(0.8,0.5,legend=paste(rep("",k-1),seq(2 + offset,k + offset,by=1),sep=""),fill=this_colors)
